"""
Original code: https://github.com/Fangyh09/pytorch-receptive-field/blob/master/torch_receptive_field/receptive_field.py
"""
import torch
    ...
    def register_hook(module):
        ...
        def hook(module, input, output):
            ...
            if not receptive_field["0"]["conv_stage"]:
                ...
                if class_name == "Conv2d" or class_name == "MaxPool2d" or class_name == "AvgPool2d" or class_name == "Conv3d" or class_name == "MaxPool3d" or class_name == 'NewMaxPool2d':
...
def receptive_field_for_unit(receptive_field_dict, layer, unit_position, verbose=True):
    ...
    if layer in receptive_field_dict:
        ...
        if np.any([unit_position[idx] < 0 or
            ...
            if len(unit_position) == 2:
                print(f'{unit_position=}, {feat_map_lim=}')
                raise Exception(f"Unit position outside spatial extent of the feature tensor ((H, W) = {tuple(feat_map_lim)}")
            else:
                raise Exception(f"Unit position outside spatial extent of the feature tensor ((D, H, W) = (%d, %d, %d)) " % tuple(feat_map_lim))
        ...
        if len(unit_position) == 2:
            ...
            _rf_range = []
            for axis in range(2):
                a = None if rf_range[axis][0] < 0 else rf_range[axis][0]
                b = None if rf_range[axis][1] > limit[axis] else rf_range[axis][1]
                _rf_range.append((a, b))
            rf_range = _rf_range
        else:
        ...
        raise KeyError(f"Layer name incorrect ({layer=}), or not included in the model. {receptive_field_dict=}")
